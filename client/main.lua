...
local function RegisterApartmentEntranceZone(apartmentID, apartmentData)
    ...
    zone:onPlayerInOut(function(isPointInside)
        if isPointInside and not InApartment then
            exports['kg-core']:DrawText(Lang:t('text.options'), 'left')
        else
...
local function RegisterApartmentEntranceTarget(apartmentID, apartmentData)
    ...
    exports['kg-target']:AddBoxZone(boxName, coords, boxData.length, boxData.width, {
        name = boxName,
...
local function RegisterInApartmentZone(targetKey, coords, heading, text)
    ...
    zone:onPlayerInOut(function(isPointInside)
        if isPointInside and text then
            exports['kg-core']:DrawText(text, 'left')
        else
            exports['kg-core']:HideText()
        end
...
local function RegisterInApartmentTarget(targetKey, coords, heading, options)
    ...
    local boxName = 'inApartmentTarget_' .. targetKey
    exports['kg-target']:AddBoxZone(boxName, coords, 1.5, 1.5, {
        name = boxName,
        heading = heading,
        minZ = coords.z - 1.0,
        maxZ = coords.z + 5.0,
...
local function DeleteApartmentsEntranceTargets()
    if Apartments.Locations and next(Apartments.Locations) then
        for id, apartment in pairs(Apartments.Locations) do
            if UseTarget then
                exports['kg-target']:RemoveZone('apartmentEntrance_' .. id)
            else
...
local function DeleteInApartmentTargets()
    ...
    if InApartmentTargets and next(InApartmentTargets) then
        for id, apartmentTarget in pairs(InApartmentTargets) do
            if UseTarget then
                exports['kg-target']:RemoveZone('inApartmentTarget_' .. id)
            else
...
local function EnterApartment(house, apartmentId, new)
    ...
    openHouseAnim()
    Wait(250)
    KGCore.Functions.TriggerCallback('apartments:GetApartmentOffset', function(offset)
        if offset == nil or offset == 0 then
            KGCore.Functions.TriggerCallback('apartments:GetApartmentOffsetNewOffset', function(newoffset)
                ...
                local data = exports['kg-interior']:CreateApartmentFurnished(coords)
                Wait(100)
                HouseObj = data[1]
                POIOffsets = data[2]
                InApartment = true
                ...
                Wait(100)
                TriggerServerEvent('kg-apartments:server:SetInsideMeta', house, apartmentId, true, false)
            ...
            TriggerEvent('kg-weathersync:client:DisableSync')
            Wait(100)
            TriggerServerEvent('kg-apartments:server:SetInsideMeta', house, apartmentId, true, true)
            TriggerServerEvent('InteractSound_SV:PlayOnSource', 'houses_door_close', 0.1)
            TriggerServerEvent('apartments:server:setCurrentApartment', CurrentApartment)
        end
        ...
        if new ~= nil then
            if new then
                TriggerEvent('kg-interior:client:SetNewState', true)
            else
                TriggerEvent('kg-interior:client:SetNewState', false)
            end
        else
            TriggerEvent('kg-interior:client:SetNewState', false)
        end
...
local function LeaveApartment(house)
    TriggerServerEvent('InteractSound_SV:PlayOnSource', 'houses_door_open', 0.1)
    openHouseAnim()
    TriggerServerEvent('kg-apartments:returnBucket')
    DoScreenFadeOut(500)
    ...
    exports['kg-interior']:DespawnInterior(HouseObj, function()
        ...
        TriggerServerEvent('kg-apartments:server:SetInsideMeta', CurrentApartment, false)
...
local function SetClosestApartment()
    ...
    if current ~= ClosestHouse and LocalPlayer.state.isLoggedIn and not InApartment then
        ...
        KGCore.Functions.TriggerCallback('apartments:IsOwner', function(result)
            IsOwned = result
            DeleteApartmentsEntranceTargets()
            DeleteInApartmentTargets()
        end, ClosestHouse)
...
function MenuOwners()
    KGCore.Functions.TriggerCallback('apartments:GetAvailableApartments', function(apartments)
        if next(apartments) == nil then
            KGCore.Functions.Notify(Lang:t('error.nobody_home'), 'error', 3500)
            CloseMenuFull()
        else
            local apartmentMenu = {
                {
            ...
            exports['kg-menu']:openMenu(apartmentMenu)
        end
    end, ClosestHouse)
end
...
RegisterNetEvent('apartments:client:setupSpawnUI', function(cData)
    KGCore.Functions.TriggerCallback('apartments:GetOwnedApartment', function(result)
        if result then
            TriggerEvent('kg-spawn:client:setupSpawns', cData, false, nil)
            TriggerEvent('kg-spawn:client:openUI', true)
            TriggerEvent('apartments:client:SetHomeBlip', result.type)
        else
            if Apartments.Starting then
                TriggerEvent('kg-spawn:client:setupSpawns', cData, true, Apartments.Locations)
                TriggerEvent('kg-spawn:client:openUI', true)
            else
                TriggerEvent('kg-spawn:client:setupSpawns', cData, false, nil)
                TriggerEvent('kg-spawn:client:openUI', true)
...
RegisterNetEvent('apartments:client:SpawnInApartment', function(apartmentId, apartment)
    ...
    if RangDoorbell ~= nil then
        ...
        if doorbelldist > 5 then
            KGCore.Functions.Notify(Lang:t('error.to_far_from_door'))
            return
        end
    end
...
RegisterNetEvent('kg-apartments:client:LastLocationHouse', function(apartmentType, apartmentId)
...
RegisterNetEvent('apartments:client:RingDoor', function(player, _)
    ...
    TriggerServerEvent('InteractSound_SV:PlayOnSource', 'doorbell', 0.1)
    KGCore.Functions.Notify(Lang:t('info.at_the_door'))
end)
