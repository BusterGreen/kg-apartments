...
RegisterNetEvent('kg-apartments:server:SetInsideMeta', function(house, insideId, bool, isVisiting)
    local src = source
    ...
    if bool then
        ...
        KGCore.Functions.SetPlayerBucket(src, tonumber(routeId))
    else
        ...
        Player.Functions.SetMetaData('inside', insideMeta)
        KGCore.Functions.SetPlayerBucket(src, 0)
    end
end)

RegisterNetEvent('kg-apartments:returnBucket', function()
    local src = source
...
RegisterNetEvent('apartments:server:openStash', function(CurrentApartment)
    local src = source
    exports['kg-inventory']:OpenInventory(src, CurrentApartment)
end)
...
RegisterNetEvent('apartments:server:CreateApartment', function(type, label)
    ...
    TriggerClientEvent('KGCore:Notify', src, Lang:t('success.receive_apart') .. ' (' .. label .. ')')
...
RegisterNetEvent('apartments:server:UpdateApartment', function(type, label)
    ...
    TriggerClientEvent('KGCore:Notify', src, Lang:t('success.changed_apart'))
    TriggerClientEvent('apartments:client:SetHomeBlip', src, type)
end)
...
RegisterNetEvent('apartments:server:OpenDoor', function(target, apartmentId, apartment)
    local OtherPlayer = KGCore.Functions.GetPlayer(target)
...
KGCore.Functions.CreateCallback('apartments:GetAvailableApartments', function(_, cb, apartment)
    local apartments = {}
...
KGCore.Functions.CreateCallback('apartments:GetApartmentOffset', function(_, cb, apartmentId)
    local retval = 0
...
KGCore.Functions.CreateCallback('apartments:GetApartmentOffsetNewOffset', function(_, cb, apartment)
...
KGCore.Functions.CreateCallback('apartments:GetOwnedApartment', function(source, cb, cid)
    if cid ~= nil then
...
KGCore.Functions.CreateCallback('apartments:IsOwner', function(source, cb, apartment)
    local src = source
    local Player = KGCore.Functions.GetPlayer(src)
    if Player ~= nil then
        local result = MySQL.query.await('SELECT * FROM apartments WHERE citizenid = ?', { Player.PlayerData.citizenid })
...
KGCore.Functions.CreateCallback('apartments:GetOutfits', function(source, cb)
    local src = source
    local Player = KGCore.Functions.GetPlayer(src)
